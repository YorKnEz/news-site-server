...
// required for generating an unique id for email verification
	...
	try {
		...
		if (MAIL_USER && MAIL_PASS) {
			...
			const mailString = fs.readFileSync(
				path.resolve("src/auth-server/views/confirmationMail.html"),
				{
					encoding: "utf8",
				}
			)
...
exports.verifyPasswordReset = async (req, res, next) => {
	try {
		...
		if (MAIL_USER && MAIL_PASS) {
			// get the template email
			const mailString = fs.readFileSync(
				path.resolve("src/auth-server/views/passwordResetMail.html"),
				{
					encoding: "utf8",
				}
			)
